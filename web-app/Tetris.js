 ...
 * @memberof Tetris
 * @param {Tetris.Game} game The initial state of a game.
 * @param {number} points The points added to score.
 * @returns {Tetris.Game} The state after a CCW rotation is attempted.
 */
const descend = function (game, points = 0) {
    ...
    game.score = Score.add_points(points, game.score)
    return R.mergeRight(game, { "position": new_position });
};
 ...
 * @memberof Tetris
 * @param {Tetris.Game} game The initial state of a game.
 * @param {number} key_down called by R.reduce or keydown pressed
 * @returns {Tetris.Game} The state after a soft drop is attempted.
 */
Tetris.soft_drop = function (game, key_down = -1) {
    if (Tetris.is_game_over(game)) {
        return game;
    }
    if (key_down != -1) {
...
Tetris.next_turn = function (game) {
    ...
    const number_of_lines_ready_to_clear = R.count(is_complete_line, locked_field);
    game.score = Score.cleared_lines(number_of_lines_ready_to_clear, game.score);
